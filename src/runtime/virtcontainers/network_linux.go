...
func (n *LinuxNetwork) addSingleEndpoint(ctx context.Context, s *Sandbox, netInfo NetworkInfo, hotplug bool) (Endpoint, error) {
	...
	isPhysical, busType := isPhysicalIface(netInfo.Iface.Name)
	var err error
	idx := len(n.eps)
	...
	if isPhysical {
		...
		endpoint, err = createPhysicalEndpoint(idx, netInfo, busType, isFVIODisabled, n.interworkingModel)
	} else {
...
func getLinkForEndpoint(endpoint Endpoint, netHandle *netlink.Handle) (netlink.Link, error) {
	...
	case *PhysicalEndpoint:
		return getLinkByName(netHandle, ep.NetworkPair().VirtIface.Name, &netlink.Device{})
	case *VethEndpoint:
		link = &netlink.Veth{}
