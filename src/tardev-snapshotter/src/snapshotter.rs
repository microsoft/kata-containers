...
const TARGET_LAYER_SNAPSHOT_REF_LABEL: &str = "containerd.io/snapshot.ref";
...
struct Store {
    ...
    salts: Option<HashMap<String, Vec<u8>>>,                // diff_id to salt
}
...
impl Store {
    ...
    fn get_salt(&self, diff_id: &str) -> Result<Vec<u8>> {
        self.salts
            .as_ref()
    ...
    fn prepare_snapshot_for_use(
        ...
        info!(
            "jiria: prepare_snapshot_for_use kind {:#?} key {} parent {} labels {:#?}",
            kind, key, parent, labels
        );
...
impl TarDevSnapshotter {
    ...
    async fn prepare_image_layer(
        ...
        trace!("in prepaer_image_lyer");
        let dir = self.store.read().await.staging_dir()?;
        trace!("in prepaer_image_lyer2");
...

        {
            ...
            labels.insert(
                SALT_LABEL.into(),
                format!("{}", general_purpose::STANDARD.encode(&salt2)),
            );
        }
...
impl Snapshotter for TarDevSnapshotter {
    ...
    async fn commit(
        ...
        let (salt, parent) = {
            ...
            if info.labels.get(TARGET_LAYER_DIGEST_LABEL).is_none() {
                return Err(Status::unimplemented(
                    "no support for commiting arbitrary snapshots",
                ));
            }

            let diff_id_result = info.labels.get(TARGET_LAYER_SNAPSHOT_REF_LABEL);
            let diff_id = match diff_id_result {
                None => return Err(Status::failed_precondition("Missing expected label")),
            ...
            trace!("Looking up salt for {diff_id}");
            let salt = store.get_salt(diff_id);
