...
use crate::registry::{self, Container, DockerConfigLayer, ImageLayer};
            ...
            || layer_media_type.eq(TAR_DIFF_LAYER_TYPE)
        {
            if layer_index < config_layer.rootfs.diff_ids.len() {
                ...
                let layer_digest = layer["digest"]
                    .as_str()
                    .context("Missing digest attribute")?;
...
async fn create_decompressed_layer_file(
    ...
    match compressed_path {
        Some(compressed_path) => {
            pull_layer_file(client, layer_digest, compressed_path).await?;
