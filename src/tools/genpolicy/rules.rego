...
CreateContainerRequest:= {"ops": ops, "allowed": true} {
    ...
    ops_builder := [] # array of possible state operations
...
state_allows(key, value) = action {
  print("state_allows: searching key = ", key, " NOT present in state = ", data)
  not data[key]
  print("state_allows: saving to state key =", key, "value =", value)
  path := concat("/", ["", key]) # state_key := "/" + key
  # save value to state
  action := {
    "op": "add",
    "path": path, 
...
state_allows(key, value) = action {
  print("state_allows: searching sandbox name value = ", value, " present in state = ", data)
  value == data[key]
