...
pub(super) async fn prepare_disk_path(
    ...
    let raw_path = format!("{host_path}.raw");
    let temp_path = format!("{raw_path}.tmp");
    let _ = tokio::fs::remove_file(&temp_path).await;
    let status = Command::new("qemu-img")
        .args(["convert", "-f", "vmdk", "-O", "raw", &host_path, &temp_path])
        .status()
        .await
        .context("failed to execute qemu-img for OpenVMM VMDK flattening")?;
    if !status.success() {
        let _ = tokio::fs::remove_file(&temp_path).await;
        return Err(anyhow!(
...
pub(crate) struct VmmInstance {
    ...
    exit_notify: Option<watch::Sender<Option<i32>>>,
...
impl VmmInstance {
    ...
    pub(super) async fn launch_with_timeout(
        &mut self,
        openvmm_path: &str,
        ...
        if let Some(log_dir) = &log_dir {
            std::fs::create_dir_all(log_dir)
                .context(format!("failed to create openvmm log dir {log_dir}"))?;
            ...
            let log_file = std::fs::OpenOptions::new()
                ...
                .open(&log_path)
                .context(format!(
                    "failed to create openvmm log file {}",
                    log_path.display()
                ))?;
            command
                .stdout(Stdio::from(
        ...
        let mut child = command
            .spawn()
            .context(format!("failed to spawn openvmm at {openvmm_path}"))?;
    ...
    pub(crate) async fn add_pcie_device(
        ...
        let host_path = prepare_disk_path(host_path, format).await?;
...
mod tests {
    ...
    async fn launch_failure_reaps_child_and_suppresses_exit_notification() {
        ...
        let (exit_notify, exit_waiter) = watch::channel(Some(9));
        let mut instance = VmmInstance::new(exit_notify);
        ...
        assert_eq!(*exit_waiter.borrow(), None);
    }

    #[tokio::test]
    async fn committed_child_exit_sends_notification() {
        let (exit_notify, mut exit_waiter) = watch::channel(None);
        let mut instance = VmmInstance::new(exit_notify);
        ...
        tokio::time::timeout(Duration::from_secs(1), exit_waiter.changed())
            .await
            .unwrap()
            .unwrap();
        assert_eq!(*exit_waiter.borrow(), Some(7));
        assert!(instance.child.is_none());
