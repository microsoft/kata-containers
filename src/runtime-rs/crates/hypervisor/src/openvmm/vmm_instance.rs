...
impl VmmInstance {
    ...
    /// hotplug port. Used for secondary pod NICs in colocated-pod scenarios.
    pub(crate) async fn add_pcie_net_device(
        &self,
        port_name: &str,
        mac_address: String,
        tap_name: String,
        ...
        self.client()?
            ...
            .await
            .map(|_| ())
            .map_err(|e| anyhow!("openvmm add_pcie_net_device RPC failed: {:?}", e))
    }
