...
impl CloudHypervisorInner {
    ...
    fn make_file_backed_memory_private(config_path: &Path) -> Result<()> {
        let data =
            fs::read(config_path).with_context(|| format!("read {}", config_path.display()))?;
        let mut config: Value = serde_json::from_slice(&data)
            .with_context(|| format!("parse {}", config_path.display()))?;
        ...
        let zone = memory
            ...
            .and_then(Value::as_object_mut)
            .ok_or_else(|| anyhow!("snapshot config missing file-backed memory zone"))?;
        zone.insert("shared".to_string(), Value::Bool(false));
    ...
    fn restore_memory_layout(config_path: &Path) -> Result<RestoreMemoryLayout> {
        ...
        match (file_backed_zones, zones.len()) {
            (0, _) => Ok(RestoreMemoryLayout::SnapshotRanges),
            (1, 1) => Ok(RestoreMemoryLayout::FileBacked),
            _ => Err(anyhow!(
                "snapshot config must contain exactly one file-backed memory zone"
            )),
        }
    }
    ...
    fn prepare_restore_files(&self, snapshot_dir: &Path) -> Result<()> {
        ...
        match Self::restore_memory_layout(&dst_config)? {
            RestoreMemoryLayout::FileBacked => {
                ...
                // template's `memory` backing file, but each clone needs a
                // private mapping of that file.
                Self::make_file_backed_memory_private(&dst_config)
...
mod tests {
    ...
    fn test_prepare_factory_restore_preserves_file_backed_memory() {
        ...
        let clh = CloudHypervisorInner {
            id: "factory-clone".to_string(),
        ...
        let template_config: Value =
            serde_json::from_slice(&fs::read(snapshot_dir.join(CLH_SNAPSHOT_CONFIG_FILE)).unwrap())
                .unwrap();
        assert_eq!(template_config["memory"]["shared"], true);
        assert_eq!(template_config["memory"]["zones"][0]["shared"], true);
        assert!(fs::symlink_metadata(vm_path.join("memory-ranges")).is_err());
    }

    #[test]
    fn test_restore_rejects_multiple_file_backed_memory_zones() {
        let root = Builder::new()
            .prefix("clh-multi-zone-restore")
            .tempdir()
            .unwrap();
        let config_path = root.path().join(CLH_SNAPSHOT_CONFIG_FILE);
        fs::write(
            &config_path,
            serde_json::to_vec(&serde_json::json!({
                "memory": {
                    "zones": [
                        { "id": "mem0", "file": "/template/memory0" },
                        { "id": "mem1", "file": "/template/memory1" },
                    ],
                },
            }))
            .unwrap(),
        ...
        let error = CloudHypervisorInner::restore_memory_layout(&config_path).unwrap_err();
        assert_eq!(
            error.to_string(),
            "snapshot config must contain exactly one file-backed memory zone"
        );
    ...
    fn test_prepare_workload_restore_links_memory_ranges() {
        ...
        let clh = CloudHypervisorInner {
            id: "restored-workload".to_string(),
            vm_path: vm_path.to_string_lossy().into_owned(),
            ..Default::default()
        };
        clh.prepare_restore_files(&snapshot_dir).unwrap();
