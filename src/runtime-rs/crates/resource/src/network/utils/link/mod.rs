...
/// Checks whether a network interface is an SR-IOV Virtual Function (VF).
///
/// Only PCI devices can be VFs — VMBus devices are always non-VF.
/// For PCI, checks for the `physfn` symlink under the device's sysfs directory.
pub fn is_vf(name: &str) -> Result<bool> {
    match get_bus_type(name)? {
        Some(BusType::Pci) => {
            let driver_info = get_driver_info(name).context("get driver info")?;
            let bdf = &driver_info.bus_info;
            let physfn_path = Path::new("/sys/bus/pci/devices").join(bdf).join("physfn");
            Ok(std::fs::read_link(&physfn_path).is_ok())
        }
        // VMBus devices do not support SR-IOV.
        Some(BusType::Vmbus) => Ok(false),
        None => Ok(false),
    }
}
