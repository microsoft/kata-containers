...
impl ShareFsMount for VirtiofsShareMount {
    ...
    async fn share_volume(&self, config: &ShareFsVolumeConfig) -> Result<ShareFsMountResult> {
        ...
        if is_watchable_mount(&config.source) {
            ...
            let host_rw_path = utils::get_host_rw_shared_path(&self.id, &self.uvm_id);
    ...
    async fn upgrade_to_rw(&self, file_name: &str) -> Result<()> {
        ...
        let host_dest = do_get_host_path(file_name, &self.id, &self.uvm_id, "", true, true);
        ...
        let host_dest = do_get_host_path(file_name, &self.id, &self.uvm_id, "", true, false);
        bind_remount(host_dest, false)
    ...
    async fn umount_rootfs(&self, config: &ShareFsRootfsConfig) -> Result<()> {
        let host_dest = do_get_host_path(&config.target, &self.id, &self.uvm_id, &config.cid, false, false);
    ...
    async fn cleanup(&self, sid: &str) -> Result<()> {
        // Unmount ro path
        let host_ro_dest = get_host_ro_shared_path(sid, &self.uvm_id);
        ...
        let host_rw_dest = get_host_rw_shared_path(sid, &self.uvm_id);
        fs::remove_dir_all(host_rw_dest).context("failed to remove rw path")?;
        // remove the host share directory
        let host_path = get_host_shared_path(sid, &self.uvm_id);
