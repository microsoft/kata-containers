...
fn canonicalize_shared_readonly_artifacts(
    ...
    for artifact in artifacts {
        ...
        artifact
            .files
            .retain(|path| !duplicate_files.contains(path));
...
pub trait Rootfs: Send + Sync {
    ...
    async fn snapshot_artifacts(
        ...
        Err(anyhow!(
            "snapshot artifacts are not supported for rootfs implementation {}",
...
mod tests {
    ...
    fn canonicalizes_shared_readonly_artifacts() {
        ...
        canonicalize_shared_readonly_artifacts(&mut artifacts, staging.path(), published.path())
            .unwrap();
