...
pub fn finalize_snapshot_config(
    ...
    if !config_path.is_file() {
        return Err(anyhow!(
            "snapshot config is unavailable: {}",
            config_path.display()
        ));
    }
...
mod tests {
    ...
    fn rewrites_one_shared_readonly_disk_for_multiple_containers() {
        ...
        let first_writable = snapshot.path().join("containers/host-first/rwlayer.img");
        let second_writable = snapshot.path().join("containers/host-second/rwlayer.img");
        fs::write(snapshot.path().join("memory-ranges"), b"memory").unwrap();
        fs::write(
            snapshot.path().join("config.json"),
            serde_json::to_vec(&serde_json::json!({
    ...
    fn removes_inactive_restored_container_disks() {
        ...
        assert!(!is_external_restored_container_disk(
            Path::new("/var/lib/kata-containers/snapshots/source/containers/deleted/rwlayer.img"),
            configured_root,
        ));
