...
use common::{
    ...
    types::{
        ...
        ContainerSnapshotInventory, ExecProcessRequest, GuestContainerId, HostContainerId,
        KillRequest, LiveContainerSnapshot, ProcessExitStatus, ProcessStateInfo, ProcessStatus,
...
impl ContainerManager for VirtContainerManager {
    ...
    async fn create_container(&self, config: ContainerConfig, spec: oci::Spec) -> Result<PID> {
        ...
        if self.restore_context.is_restore().await {
            ...
            let host_id = HostContainerId::new(&config.container_id);
            let action = match self
                .restore_context
                .classify_create(&host_id, &cri_name, is_pause, &identity)
                .await
            {
                Ok(action) => action,
                Err(error) => {
    ...
    async fn kill_process(&self, req: &KillRequest) -> Result<()> {
        ...
        if is_termination_signal(req.signal) {
            if let Some(guest_id) = self
                .restore_context
                .pause_guest_id_for_target(&HostContainerId::new(req.process.container_id()))
                .await
            {
                let containers = self.containers.read().await;
                ...
                let guest_process =
                    ContainerProcess::new(guest_id.as_str(), req.process.exec_id())?;
                return container
    ...
    async fn prepare_restored_container(
        ...
        let container = containers
            .get(host_id.as_str())
            .ok_or_else(|| Error::ContainerNotFound(host_id.as_str().to_string()))?;
        ...
        let mappings = self
            ...
            .ok_or_else(|| {
                anyhow!(
                    "restored mount state is unavailable for {}",
                    host_id.as_str()
                )
            })?;
        ...
        for mapping in mappings {
            let mount = spec
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "target container {} has no mount at {}",
                        host_id.as_str(),
                        mapping.destination
                    )
                })?;
    ...
    async fn complete_synthetic_init(&self, process: &ContainerProcess) -> Result<()> {
        ...
        let Some(exit_code) = self
            .restore_context
            .take_synthetic_exit_code(&HostContainerId::new(process.container_id()))
            .await
        else {
            return Ok(());
        };
    ...
    async fn start_process(&self, process: &ContainerProcess) -> Result<PID> {
        if process.exec_id().is_empty() {
            ...
            if self.restore_context.is_synthetic_completed(&host_id).await {
                return self.mark_restored_container_running(&host_id).await;
            }
        }
        let containers = self.containers.read().await;
        ...
        for (host_id, container) in containers.iter() {
            ...
            match snapshot_container_lifecycle(host_id, &spec, &state)? {
                SnapshotContainerLifecycle::Live {
                    ...
                    let node_local_mounts = match self
                        .restore_context
                        .guest_mounts_for_target(&HostContainerId::new(host_id))
                        .await
                    {
