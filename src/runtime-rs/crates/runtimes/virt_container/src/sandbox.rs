...
use common::types::{ContainerProcess, GuestContainerId, HostContainerId};
use common::{
...
fn load_restore_manifest(snapshot_dir: &Path, package_version: &str) -> Result<SnapshotManifest> {
    ...
    if runtime_version.runtime_version != package_version {
        return Err(anyhow!(
    ...
    if manifest.hypervisor != "cloud-hypervisor" || manifest.source_sandbox_id.is_empty() {
...
mod snapshot_manifest_tests {
    ...
    const TEST_PACKAGE_VERSION: &str = "4.1.0.kata0-7";
    ...
    fn write_restore_fixture(root: &Path) -> SnapshotManifest {
        ...
        SnapshotManifest {
            runtime_version: TEST_PACKAGE_VERSION.to_string(),
    ...
    fn agent_transport_contract_is_required_in_manifest() {
        ...
        assert_eq!(value["runtime_version"], TEST_PACKAGE_VERSION);
    ...
    fn restore_manifest_requires_disk_ids_and_declared_files() {
        ...
        let loaded = load_restore_manifest(snapshot.path(), TEST_PACKAGE_VERSION).unwrap();
        ...
        .unwrap();
        assert!(load_restore_manifest(snapshot.path(), TEST_PACKAGE_VERSION).is_err());
    }
    ...
    fn restore_manifest_dispatches_versions_before_full_parse() {
        ...
        let error = load_restore_manifest(snapshot.path(), TEST_PACKAGE_VERSION).unwrap_err();
        assert_eq!(
            error.to_string(),
            format!(
        ...
        fs::write(
            manifest_path,
            serde_json::to_vec(&serde_json::json!({
                "runtime_version": TEST_PACKAGE_VERSION,
    ...
    fn shared_readonly_disk_restores_once_for_multiple_containers() {
        ...
        manifest
            .live_containers
            .push(SnapshotLiveContainerManifest {
                cri_name: "app".to_string(),
        ...
        let rootfs_configs =
            restored_rootfs_configs(snapshot.path(), &private_dir, &manifest).unwrap();
...
impl VirtSandbox {
                ...
                tokio::select! {
                    ...
                    res = agent.get_oom_event(agent::Empty::new()) => {
                        match res.context("get oom event") {
                            Ok(resp) => {
                                let guest_id = GuestContainerId::new(resp.container_id);
                                ...
                                let cid = restore_context
                                    ...
                                    .await
                                    .unwrap_or_else(|| HostContainerId::new(guest_id.as_str()));
                                warn!(sl!(), "send oom event for container {}", &cid);
                                let event = TaskOOM {
                                    container_id: cid.as_str().to_string(),
                                    ..Default::default()
                                };
    ...
    pub(crate) async fn new(
        ...
        Ok(Self {
            sid: sid.to_string(),
            package_version: sandbox_config.package_version.clone(),
    ...
    async fn start_restore_if_requested(
        ...
        let live_slots = manifest
            ...
            .map(|container| RestoreLiveSlot {
                cri_name: container.cri_name.clone(),
                guest_id: GuestContainerId::new(&container.snapshot_guest_id),
    ...
    async fn activate_restore_transaction(
        ...
        if !self
            .restore_context
            .begin_activation(&HostContainerId::new(target_id))
            .await?
        {
            return Ok(false);
        }
        ...
        let result: Result<()> = async {
            ...
            self.agent
                .resume_container(agent::ContainerID {
                    container_id: pause_guest_id.into_string(),
                })
                .await
    ...
    async fn create_portable_snapshot(
        ...
        let publication: Result<()> = (|| {
            ...
            let completed_containers = inventory
                ...
                .collect::<Result<Vec<_>>>()?;
            let manifest = SnapshotManifest {
                runtime_version: self.package_version.clone(),
                format_version: SNAPSHOT_MANIFEST_FORMAT_VERSION,
                hypervisor: "cloud-hypervisor".to_string(),
...
impl Persist for VirtSandbox {
    ...
    async fn restore(
        ...
        Ok(Self {
            sid: sid.to_string(),
            package_version: sandbox_args.package_version,
