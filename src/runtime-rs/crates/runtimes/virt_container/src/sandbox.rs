...
fn restore_source_from_annotations(
    ...
    let root_metadata = fs::symlink_metadata(snapshot_root).with_context(|| {
        format!(
            "snapshot_root is unavailable: {}",
            snapshot_root.display()
        )
    })?;
...
mod snapshot_manifest_tests {
    ...
    fn restore_source_rejects_unsafe_root_entries() {
        ...
        fs::write(snapshot_root.path().join("busybox-kata"), b"not a directory").unwrap();
