...
use common::types::{GuestContainerId, HostContainerId};
...
struct RestoreState {
    ...
    source_pause_guest_id: Option<GuestContainerId>,
    target_pause_id: Option<HostContainerId>,
}
...
impl RestoreContext {
    ...
    pub(crate) async fn begin(
        ...
        source_pause_guest_id: GuestContainerId,
    ) -> Result<()> {
        if source_sandbox_id.is_empty() || source_pause_guest_id.as_str().is_empty() {
        ...
        state.source_pause_guest_id = Some(source_pause_guest_id);
        Ok(())
    }
    ...
    pub(crate) async fn adopt_pause(
        &self,
        ...
        match state.target_pause_id.as_ref() {
            None => state.target_pause_id = Some(target_id),
    ...
    pub(crate) async fn pause_guest_id_for_target(
        ...
        (state.target_pause_id.as_ref() == Some(target_id))
...
mod tests {
    ...
    async fn activates_only_the_target_sandbox() {
        ...
        context
            .begin("source", GuestContainerId::new("source-pause"))
            .await
            .unwrap();
        context.prepared_paused().await.unwrap();
        let target = HostContainerId::new("target");
        assert!(context.adopt_pause(target.clone(), true).await.unwrap());
    ...
    async fn rejects_workload_adoption_in_phase_three() {
        ...
        context.prepared_paused().await.unwrap();
        assert!(context
            .adopt_pause(HostContainerId::new("workload"), false)
            .await
            .is_err());
    }
}
