...
use common::types::{CompletedContainerSnapshot, GuestContainerId, HostContainerId};
...
pub(crate) struct RestoreLiveSlot {
    ...
    pub(crate) guest_id: GuestContainerId,
...
struct LiveSlotState {
    ...
    claimed_host_id: Option<HostContainerId>,
...
pub(crate) struct RestoreContext {
    target_sandbox_id: HostContainerId,
...
impl RestoreContext {
    pub(crate) fn new(target_sandbox_id: &str) -> Self {
        Self {
            target_sandbox_id: HostContainerId::new(target_sandbox_id),
    ...
    pub(crate) async fn classify_create(
        ...
        if target_id.as_str().is_empty() || cri_name.is_empty() {
        ...
        if let Some(slot) = state.completed_slots.get_mut(cri_name) {
            ...
            match slot.claimed_host_id.as_ref() {
                None => slot.claimed_host_id = Some(target_id.clone()),
        ...
        if let Some(existing) = state
            .host_to_guest
            .insert(target_id.clone(), guest_id.clone())
        {
        ...
        if let Some(existing) = state
            .guest_to_host
            .insert(guest_id.clone(), target_id.clone())
        {
    ...
    pub(crate) async fn begin_activation(&self, target_id: &HostContainerId) -> Result<bool> {
    ...
    pub(crate) async fn source_pause_guest_id(&self) -> Result<GuestContainerId> {
        self.state
            .lock()
            .await
            .live_slots
    ...
    pub(crate) async fn target_pause_id(&self) -> Option<HostContainerId> {
        self.state
            .lock()
            .await
            .live_slots
    ...
    pub(crate) async fn pause_guest_id_for_target(
        &self,
        target_id: &HostContainerId,
    ) -> Option<GuestContainerId> {
        self.resolve_guest_id(target_id).await
    }

    pub(crate) async fn resolve_guest_id(
        &self,
    ...
    pub(crate) async fn resolve_host_id(
        &self,
        guest_id: &GuestContainerId,
    ) -> Option<HostContainerId> {
    ...
    pub(crate) async fn guest_mounts_for_target(
        ...
        self.state
            ...
            .live_slots
            .values()
            .find(|slot| slot.claimed_host_id.as_ref() == Some(host_id))
            .map(|slot| slot.node_local_mounts.clone())
    }

    pub(crate) async fn is_synthetic_completed(&self, host_id: &HostContainerId) -> bool {
        self.state
            .lock()
            .await
            .completed_slots
            .values()
            .any(|slot| slot.claimed_host_id.as_ref() == Some(host_id))
    }

    pub(crate) async fn take_synthetic_exit_code(&self, host_id: &HostContainerId) -> Option<i32> {
        ...
        let slot = state.completed_slots.values_mut().find(|slot| {
            slot.claimed_host_id.as_ref() == Some(host_id) && slot.completion_pending
        })?;
        slot.completion_pending = false;
        Some(slot.exit_code)
    }

    pub(crate) async fn retire_synthetic_completed(&self, host_id: &HostContainerId) {
...
mod tests {
    ...
    fn live_slot(name: &str, guest_id: &str, value: &str) -> RestoreLiveSlot {
        RestoreLiveSlot {
            ...
            guest_id: GuestContainerId::new(guest_id),
            identity: identity(value),
            node_local_mounts: Vec::new(),
        }
    }

    fn host(value: &str) -> HostContainerId {
        HostContainerId::new(value)
    }

    fn guest(value: &str) -> GuestContainerId {
        GuestContainerId::new(value)
    }
    ...
    async fn activates_only_the_target_sandbox() {
        ...
        assert_eq!(
            ...
            RestoreCreateAction::AdoptLive {
                guest_id: guest("source-pause"),
                is_pause: true,
            }
        );
        assert!(context.begin_activation(&host("target")).await.unwrap());
        ...
        assert!(!context.begin_activation(&host("workload")).await.unwrap());
    }
    ...
    async fn live_claim_maps_host_and_guest_ids_once() {
        ...
        assert_eq!(
            context
                ...
                .unwrap(),
            RestoreCreateAction::AdoptLive {
                guest_id: guest("source-app"),
                is_pause: false,
            }
        );
        assert_eq!(
            context.resolve_guest_id(&host("target-app")).await,
            Some(guest("source-app"))
        );
        assert_eq!(
            context.resolve_host_id(&guest("source-app")).await,
            Some(host("target-app"))
        );
        assert_eq!(
        ...
        assert_eq!(
            context.guest_mounts_for_target(&host("cold-app")).await,
            None
        );
        assert!(context
    ...
    async fn completed_slot_finishes_once_then_allows_cold_replacement() {
        ...
        assert_eq!(
            context
                .classify_create(&host("replacement"), "setup", false, &identity("setup"))
                .await
                .unwrap(),
            RestoreCreateAction::Cold
        );
    ...
    async fn persisted_state_preserves_activation_claims_and_stable_ids() {
        ...
        assert_eq!(
            restored.resolve_guest_id(&host("target")).await,
            Some(guest("source-pause"))
        );
        assert_eq!(
