...
impl KataAgent {
    ...
    pub(crate) async fn start_connection(
        ...
        let (previous, connecting) = {
            ...
            } else if previous.status != ConnectionStatus::Disconnected {
        ...
        if let Err(error) = result {
            ...
            self.set_connection(previous).await;
            return Err(error);
        }
...
mod tests {
    ...
    async fn close_client_cancels_requests_held_by_cloned_clients() {
        ...
        let client_socket = tokio::net::UnixStream::from_std(client_socket).unwrap();
        let client_fd = client_socket.as_raw_fd();
        let client = Client::new(TtrpcSocket::from(client_socket));
        {
