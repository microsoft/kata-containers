...
impl EztHandler {
    ...
    pub fn start_ezt_handler(&mut self, sndbx_id: String) ->Result<()> {
        ...
        if Path::new(NETWORK_POLICY_FILE).exists() {
            let policy_data = parse_network_policy()?;
            ...
            self.handle.push(task::spawn(listen_task(async_listener_sock, sndbx_id.clone(),policy_data, pod_selected)));
...
fn handle_port(ingress_ports: HashMap<String, Vec<u32>>, egress_ports: HashMap<String, Vec<u32>>) -> Result<()> {
    warn!(sl(), "EZT: Handle port");

    let mut ingress_ports_tcp = String::from("");
    let mut egress_ports_tcp = String::from("");
    let mut ingress_ports_udp = String::from("");
    let mut egress_ports_udp = String::from("");
...
fn run_ezt_script() -> Result<InterfaceSettings>{
    let script_path = "/tmp/wgsetup.sh";
...
async fn listen_task(sock: TokioUdpSocket, sndbx_id: String, policy: NetworkPolicy, pod_selected: bool) {
                ...
                match String::from_utf8(data.to_vec()) {
                    Ok(payload) => {
                        ...
                        if v.len() != 8 {
                            ...
                            // If this pod is selected, match the receiving label against ingress/egress block
                            if pod_selected {
                                let peer_label = v[5].to_string();
                                let peer_label_split: Vec<&str> = peer_label.split(':').collect();
                                if peer_label_split.len() == 2 {
                                    warn!(sl(), "EZT: listen_task: peer labels received: {}:{}", peer_label_split[0], peer_label_split[1]);
                                    if policy.is_peer_allowed((peer_label_split[0], peer_label_split[1])) {
                                        warn!(sl(), "Setting peer for this label");
...
async fn send_task(sock: TokioUdpSocket, addr: SocketAddr, mut sndbx_id: String, ezt_if: InterfaceSettings, podlabel: (String, String)) {
    ...
    let label_data = format!(":{}:{}:{}:{}:{}:{}:test=Multicast", ezt_if.eth_ip, ezt_if.wg_port, allowed_ip, ezt_if.wg_pub_key, label,ezt_if.veth_defns_end_ip);
