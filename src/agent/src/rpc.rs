...
use protocols::agent::{
    ...
    GuestDetailsResponse, Interfaces, Metrics, MountRequest, OOMEvent, ReadStreamResponse, Routes,
...
impl agent_ttrpc::AgentService for AgentService {
    ...
    async fn mount(
        &self,
        ctx: &TtrpcContext,
        req: protocols::agent::MountRequest,
    ) -> ttrpc::Result<Empty> {
        trace_rpc_call!(ctx, "mount", req);
        is_allowed(&req).await?;

        do_mount(&req).await.map_ttrpc_err(same)?;
...
async fn receive_mount_file(req: &MountRequest) -> Result<()> {
    let guest_mount_source = if let Some(p) = MOUNT_STATE
        .lock()
        .await
        .get_mapping(&req.container_id, &req.host_mount_source)
    {
