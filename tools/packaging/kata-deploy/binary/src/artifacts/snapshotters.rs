...
pub async fn configure_erofs_snapshotter(config: &Config, configuration_file: &Path) -> Result<()> {
    ...
    // Both keys must be written on every path. The snapshotter decides whether a
    // layer is mounted through device-mapper; the differ decides whether the hash
    // tree is built at all. Setting only the former yields a layer that claims
    // dm-verity but carries no verity metadata, which is exactly the "protected in
    // name only" case RM-40 refuses. Writing "off"/"false" explicitly also clears a
    // stale enabled value left by a previous deployment.
    toml_utils::set_toml_value(
        configuration_file,
