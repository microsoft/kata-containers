...
async fn configure_mariner(config: &Config) -> Result<()> {
    ...
    // configure_mariner mutates configuration-clh.toml (and the runtime-rs
    // sibling configuration-clh-runtime-rs.toml) in place to point them at
    // the cbl-mariner-shipped cloud-hypervisor binary (and to flip a couple
    // of related knobs). It is a no-op for hosts that do not have a clh
    // shim enabled — for example, an openvmm-only deployment must not have
    // its `hypervisor_name = "openvmm"` rewritten to `clh`. Gate on the
    // active shim list to keep this function clh-specific.
    if !config
        .shims_for_arch
        .iter()
        .any(|s| s == "clh" || s == "clh-runtime-rs")
    {
        return Ok(());
    }
    ...
    for config_path in config_paths {
        ...
        let clh_path = format!("{}/bin/cloud-hypervisor", config.dest_dir);
