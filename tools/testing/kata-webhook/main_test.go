...
func TestAnnotatePodMutator(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "limit equal to minimum is left alone",
			minMemory:     enforcedLimit,
			setMinMemory:  true,
		...
		{
			name:          "limit above minimum is left alone",
			minMemory:     enforcedLimit,
			setMinMemory:  true,
		...
		{
			name:         "container without a memory limit is left alone",
			minMemory:    enforcedLimit,
			setMinMemory: true,
			container: &corev1.Container{
		...
		{
			name:          "container without any resources is left alone",
			minMemory:     enforcedLimit,
			setMinMemory:  true,
			container:     &corev1.Container{},
			wantMutated:   true,
