...
make PACKAGE_VERSION=<distro-package-release> all
sudo make deploy
popd
```
...
```
pushd kata-containers/tools/osbuilder/node-builder/azure-linux
make PACKAGE_VERSION=<distro-package-release> all-confpods
...
Notes:
  - `PACKAGE_VERSION` is required when building packages and must identify the
    ...
    restore incompatible snapshots silently. This is the Kata package version,
    not the host OS version. Direct runtime-rs builds use the repository's
    `VERSION` unless this value is overridden.
...
```shell
sudo make PACKAGE_VERSION=<distro-package-release> BUILD_TYPE=debug all-confpods deploy-confpods
```
...
```shell
sudo make PACKAGE_VERSION=<distro-package-release> BUILD_TYPE=debug SHIM_USE_DEBUG_CONFIG=no all-confpods deploy-confpods
```
...
```shell
sudo make PACKAGE_VERSION=<distro-package-release> SHIM_REDEPLOY_CONFIG=no all-confpods deploy-confpods
```
...
```shell
sudo make PACKAGE_VERSION=<distro-package-release> BUILD_TYPE=debug SHIM_REDEPLOY_CONFIG=no all-confpods deploy-confpods
```
