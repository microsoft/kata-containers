...
> [!WARNING]
> This is a bring-up configuration, not a production-ready deployment. The
> development agent policy is permissive and the guest layout depends on a
> fixed-GPA workaround. Only the diagnostic IGVM enables confidential debugging.
...
Rootfs mode requires dm-verity metadata and rejects a command line that cannot
be updated to enable dm-verity.
...
sudo dnf install -y \
    ...
    protobuf-devel python3-pip qemu-img tar
```
...
- `default_vcpus` and `static_sandbox_default_workload_vcpus` in the generated
  runtime-rs configuration.

Static sizing uses workload limits when supplied and the configured workload
defaults otherwise. The SNP configuration adds no CPU or memory overhead.
Explicit CPU limits must still match the VP count embedded in the IGVM.
...
- The diagnostic manifest enables confidential debugging; the Kata rootfs
