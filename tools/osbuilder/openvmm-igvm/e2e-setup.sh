...
sed \
	...
	-e "s/^static_sandbox_default_workload_vcpus = .*/static_sandbox_default_workload_vcpus = ${vp_count}/" \
...
if sudo test -e "${containerd_config}"; then
	sudo cp -a "${containerd_config}" "${containerd_config}.bak.$(date +%Y%m%d-%H%M%S)"
fi
if sudo test -e "${containerd_dropin}"; then
	sudo cp -a "${containerd_dropin}" "${containerd_dropin}.bak.$(date +%Y%m%d-%H%M%S)"
fi
