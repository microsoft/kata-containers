...
# This is to avoid the following error in the msft fork:
...
function TestContainerMemoryUpdate() {
	...
	for virtio_mem_enabled in 1 0; do
		...
		# TODO(msft fork): root-cause and re-enable the memory decrease check.
		# Passing 0 keeps the memory increase assertions running and skips only the
		# decrease assertion
		DoContainerMemoryUpdate 0
	done
}
