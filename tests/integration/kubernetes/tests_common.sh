...
hard_coded_policy_tests_enabled() {
	...
	declare -r enabled_hypervisors=("qemu-coco-dev" "qemu-sev" "qemu-snp" "qemu-tdx")
	...
	[[ "${enabled}" == "no" && "${KATA_HOST_OS}" == "cbl-mariner" ]] && enabled="yes"
	[[ "${enabled}" == "no" && auto_generate_policy_enabled ]] && enabled="yes"
