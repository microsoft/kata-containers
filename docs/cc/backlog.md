...
| RM-6 | ✅ **Test the SRM integration, not just the crate.** The four `rpc.rs` call sites had no test coverage at all — every reference-monitor test was crate-level, and the rpc test module never touched `SRM`. Added a `rpc::tests::srm_integration` module (7 tests) covering operation-id injectivity, `commit_or_quarantine` on both paths, `retire_or_warn`, and the remove-frees-the-create-id sequence. | The crate is well tested and the defects keep being found in the wiring: F-19 (removal never wrapped), F-23 (replay cache misapplied), RM-3 (results discarded), and now F-24 (colliding operation ids), which this work found. Crate-level tests cannot see any of those. |
| RM-7 | **Define what the caller is told when the monitor quarantines.** On a `commit` failure the RPC returns success (the operation did succeed) and quarantines. The shim then learns the guest is degraded only when its *next* SRM-gated call fails with an opaque `FAILED_PRECONDITION`, with no way to distinguish "this request was invalid" from "the guest is no longer able to prove anything". Options: a distinct error code for quarantine, a health/status field the shim can poll, or an explicit event. | Quarantine is the fail-closed backstop for the whole of FR-6, but nothing propagates it. A host that cannot tell a rejected request from a dead enforcer will retry, and the retries look identical to an attack. Deciding the contract also settles whether returning success on commit failure is the right answer or merely the least-bad one. |
| RM-8 | **Decide whether teardown is exempt from quarantine.** `prepare` refuses every operation once quarantined, and `SignalProcess` and `RemoveContainer` are both transactions — so a quarantined monitor cannot gracefully stop or remove a container. Only sandbox-level destroy, which is not SRM-gated, still works. Either exempt teardown operations explicitly, or accept sandbox destruction as the only recovery and say so. | Wrapping removal in a transaction was correct, but it changed quarantine's blast radius as a side effect: the code comment claiming quarantine "leaves teardown paths usable" became false without anything failing. This is exactly the drift a per-feature `Limits` bullet (DOC-3) is meant to catch. |
