...
| `E2E_ALLOW_UNATTESTED_INITDATA` | `yes` on `qemu-coco-dev`, `no` elsewhere | Builds the agent with the `allow-unattested-initdata` feature, which downgrades the F-166 "initdata is not bound to a launch measurement" abort to a warning. A non-confidential hypervisor has no TEE report provider, so without this every sandbox carrying initdata aborts about three seconds into boot, and the only symptom is a vsock connect timeout that reads like a host fault (F-213). Never set it on a TEE platform and never ship a build carrying it: it lets the host supply unmeasured policy and SRM trust roots. Stage 04 asserts the built binary matches whichever direction you asked for. |
